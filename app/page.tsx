...
export default function Home() {
  ...
  const formatYazi = (str: string) => {
    if (!str || str.toLowerCase() === "bilinmiyor") return "Belirtilmemiş";
  ...
  const veriyiDuzenle = (item: any) => {
    ...
    // 1. MARKA & MODEL (Chery ve diğerleri için anahtar kelime eşleştirme)
    const mName = d.marka || d.Marka || "";
    const modName = d.model || d.Model || "";
    d.marka_format = formatYazi(mName);
    d.model_format = formatYazi(modName);

    // 2. İSİM GİZLEME (K.V. veya Veri yazısını kaldırıp gerçek baş harfleri koyar)
    const asilIsim = d.ad_soyad || d.isim || d.Kullanıcı || "";
    if (asilIsim && asilIsim.length > 0) {
      d.bas_harfler = asilIsim.trim().split(/\s+/).map((p: string) => p.charAt(0).toUpperCase() + ".").join(" ");
    } else {
      d.bas_harfler = "Y. K."; // Veride isim yoksa anonim bir kısaltma
    }

    // 3. NOT TEMİZLEME (İsim ifşasını önler)
    let temizNot = d.not || "";
    d.temiz_not = temizNot.replace(/\b([A-ZÇĞİÖŞÜ])[a-zçğıöşüöü]+\s+([A-ZÇĞİÖŞÜ])[a-zçğıöşüöü]+\b/g, "$1. $2.");

    // 4. FİYAT (Fiyat Alınız hatasını çözer)
  ...
  const tumMarkalar = Array.from(new Set(islenmisVeri.map(item => item.marka_format))).filter(m => m !== "Belirtilmemiş").sort();

  useEffect(() => {
    if (secilenMarka) {
      const modeller = Array.from(new Set(islenmisVeri.filter(item => item.marka_format === secilenMarka).map(item => item.model_format))).filter(m => m !== "Belirtilmemiş").sort();
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-20 text-left relative">
      ...
      <div className="relative h-[60vh] flex items-center justify-center px-6 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center w-full">
          ...
          <div className="bg-white p-5 rounded-[3rem] shadow-2xl grid grid-cols-1 md:grid-cols-4 gap-4 text-left">
              ...
              <select value={secilenSehir} onChange={(e) => setSecilenSehir(e.target.value)} className="p-4 bg-slate-50 border-0 rounded-2xl font-bold outline-none cursor-pointer"><option value="">Şehir Seçin</option>{Array.from(new Set(islenmisVeri.map(i => i.sehir))).filter(s => s && s !== "bilinmiyor").sort().map(s => <option key={s} value={s}>{s}</option>)}</select>
          ...
          <div key={item.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm hover:border-blue-300 transition-all text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>
            <div className="p-8 md:p-12 flex flex-col md:flex-row items-center">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 w-full font-black uppercase italic text-left">
                  ...
                  <div className="flex flex-col text-left"><span className="text-[11px] text-slate-300 mb-2 uppercase">Tarih</span><p className="text-base text-slate-500">{item.tarih || "Şubat 2026"}</p></div>
                  <div className="flex flex-col items-end md:items-start text-left"><span className="text-[11px] text-slate-300 mb-2 uppercase">Tutar</span><p className="text-4xl font-black text-blue-700 tracking-tighter">{item.ekran_fiyat}</p></div>
                </div>
            </div>
            {acikKartId === item.id && (
              <div className="p-10 bg-slate-50 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm italic text-left animate-in slide-in-from-top-4">
                <div className="space-y-2 uppercase text-left"><p className="text-[10px] font-black text-slate-400 tracking-widest border-b pb-1 mb-2">Detaylar</p><p><b>Motor:</b> {formatYazi(item.motor)}</p><p><b>KM:</b> {formatYazi(item.km)}</p></div>
                <div className="space-y-2 uppercase text-left"><p className="text-[10px] font-black text-slate-400 tracking-widest border-b pb-1 mb-2">Servis Bilgisi</p><p><b>Servis:</b> {item.servis_adi || "Belirtilmemiş"}</p></div>
      ...
      {/* MODAL VE BLOG ALANI AYNI ŞEKİLDE DEVAM EDİYOR */}
    </main>
  );
}
