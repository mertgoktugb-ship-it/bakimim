...
export default function Home() {
  ...
  const jsonCiktisiAl = () => {
    ...
    alert("Güncel JSON kopyalandı! Dosyaya yapıştırın.");
  };
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-20 text-left relative">
          ...
          <div key={item.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm hover:border-blue-300 transition-all text-left">
            <div className="p-8 md:p-12 flex flex-col md:flex-row items-center text-left relative">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 w-full font-black uppercase italic text-left">
                  <div className="flex flex-col text-left">
                    <span className="text-[11px] text-slate-300 mb-2 uppercase text-left">Bakım</span>
                    {duzenlemeId === item.id ? <input className="bg-slate-100 p-1 rounded" value={item.bakim_turu} onChange={(e) => hucreGuncelle(item.id, 'bakim_turu', e.target.value)} /> : <p className="text-base text-slate-700 text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>{item.bakim_turu}</p>}
                  </div>
                  <div className="flex flex-col text-left">
                    <span className="text-[11px] text-slate-300 mb-2 uppercase text-left">Konum</span>
                    {duzenlemeId === item.id ? <input className="bg-slate-100 p-1 rounded" value={item.sehir} onChange={(e) => hucreGuncelle(item.id, 'sehir', e.target.value)} /> : <p className="text-base text-slate-700 text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>{item.sehir}</p>}
                  </div>
                  <div className="flex flex-col text-left">
                    <span className="text-[11px] text-slate-300 mb-2 uppercase text-left">Tarih</span>
                    {duzenlemeId === item.id ? <input className="bg-slate-100 p-1 rounded" value={item.tarih} onChange={(e) => hucreGuncelle(item.id, 'tarih', e.target.value)} /> : <p className="text-base text-slate-500 text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>{item.tarih}</p>}
                  </div>
                  <div className="flex flex-col items-end md:items-start text-left">
                    <span className="text-[11px] text-slate-300 mb-2 uppercase text-left text-left">Tutar</span>
                    {duzenlemeId === item.id ? <input className="bg-slate-100 p-1 rounded" value={item.fiyat} onChange={(e) => hucreGuncelle(item.id, 'fiyat', e.target.value)} /> : <p className="text-4xl font-black text-blue-700 tracking-tighter text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>{item.ekran_fiyat}</p>}
                  </div>
                </div>
            </div>
            ...
            {acikKartId === item.id && (
              <div className="p-10 bg-slate-50 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm italic text-left">
                ...
                {duzenlemeId === item.id ? (
                  <>Motor: <input className="bg-white p-1 rounded w-full mb-1" value={item.motor} onChange={(e) => hucreGuncelle(item.id, 'motor', e.target.value)} />
                  KM: <input className="bg-white p-1 rounded w-full" value={item.km} onChange={(e) => hucreGuncelle(item.id, 'km', e.target.value)} /></>
                ) : (<><p className="text-left text-left"><b>Motor:</b> {item.motor || '-'}</p><p className="text-left text-left"><b>KM:</b> {item.km}</p></>)}
                </div>
                <div className="space-y-2 uppercase text-left"><p className="text-[10px] font-black text-slate-400 tracking-widest border-b pb-1 mb-2 text-left text-left text-left">Servis</p>
                {duzenlemeId === item.id ? <input className="bg-white p-1 rounded w-full" value={item.servis_adi} onChange={(e) => hucreGuncelle(item.id, 'servis_adi', e.target.value)} /> : <p className="text-left text-left"><b>Adı:</b> {item.servis_adi}</p>}
                </div>
                <div className="bg-gradient-to-br from-[#0f172a] to-[#1e293b] text-white p-6 rounded-[2rem] shadow-lg text-left text-left">
                {duzenlemeId === item.id ? <textarea className="bg-white/10 w-full p-2 rounded" value={item.not} onChange={(e) => hucreGuncelle(item.id, 'not', e.target.value)} /> : <span>"{item.not || "Doğrulanmış fatura kaydıdır."}"</span>}
                </div>
              </div>
            )}
          </div>
      ...
      <section className="max-w-5xl mx-auto px-6 mt-32 mb-20 pt-20 border-t border-slate-200 text-left text-left text-left text-left text-left text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
          {blogYazilari.map((blog) => (
            <Link key={blog.id} href={`/blog/${blog.id}`} className="group text-left">
      ...
      <footer className="bg-white border-t border-slate-200 py-20 px-8 text-left text-left text-left text-left text-left">
