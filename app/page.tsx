...
export default function BakimimApp() {
  ...
  // YENİ: Sıralama durumu (Varsayılan olarak en yeni tarih)
  ...
  // YENİ: Sonuçları seçilen duruma göre anlık sıralama işlemi
  const siraliSonuclar = [...sonuclar].sort((a, b) => {
    if (siralama === "fiyat-artan") return (a.fiyat || 0) - (b.fiyat || 0);
    if (siralama === "fiyat-azalan") return (b.fiyat || 0) - (a.fiyat || 0);
    if (siralama === "tarih-yeni") return new Date(b.tarih || 0).getTime() - new Date(a.tarih || 0).getTime();
    if (siralama === "tarih-eski") return new Date(a.tarih || 0).getTime() - new Date(b.tarih || 0).getTime();
    if (siralama === "model-a-z") return (a.model_format || "").localeCompare(b.model_format || "");
    return 0;
  });
    ...
    <main className={`min-h-screen pb-20 text-left relative font-sans transition-colors duration-500 ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-[#F8FAFC] text-slate-800'}`}>
      ...
      {veriYukleniyor ? (
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* YENİ: SIRALAMA BAR ÇUBUĞU */}
          {siraliSonuclar.length > 0 && (
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 animate-in fade-in duration-500">
              <div className={`text-[10px] font-black uppercase tracking-widest py-2 px-4 rounded-xl ${isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                {siraliSonuclar.length} Kayıt Bulundu
              </div>
