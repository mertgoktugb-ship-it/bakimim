...
import { 
  Car, Search, BookOpen, ArrowRight, CheckCircle2, Lock, Settings 
} from 'lucide-react';
    ...
    <main className="min-h-screen bg-[#F8FAFC] pb-20 text-left relative">
      ...
      <div className="relative pt-32 pb-48 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          <h1 className="text-6xl md:text-8xl font-black text-white mb-12 uppercase italic tracking-tighter leading-none">FİYAT <span className="text-blue-500 font-black">KIYASLA</span></h1>
          <div className="bg-white p-5 rounded-[3rem] shadow-2xl grid grid-cols-1 md:grid-cols-4 gap-4">
              <select value={secilenMarka} onChange={(e) => setSecilenMarka(e.target.value)} className="p-4 bg-slate-50 border-0 rounded-2xl font-bold outline-none cursor-pointer text-left"><option value="">Marka</option>{tumMarkalar.map(m => <option key={m} value={m}>{m}</option>)}</select>
              <select value={secilenModel} onChange={(e) => setSecilenModel(e.target.value)} className="p-4 bg-slate-50 border-0 rounded-2xl font-bold outline-none cursor-pointer text-left"><option value="">Model</option>{musaitModeller.map(m => <option key={m} value={m}>{m}</option>)}</select>
              <select value={secilenSehir} onChange={(e) => setSecilenSehir(e.target.value)} className="p-4 bg-slate-50 border-0 rounded-2xl font-bold outline-none cursor-pointer text-left"><option value="">Şehir</option>{tumSehirler.map(s => <option key={s} value={s}>{s}</option>)}</select>
          ...
          <div key={item.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm hover:border-blue-300 transition-all">
            <div className="p-8 md:p-12 flex flex-col md:flex-row items-center cursor-pointer text-left" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 w-full font-black uppercase italic text-left">
                  <div className="flex flex-col text-left"><span className="text-[11px] text-slate-300 mb-2 tracking-widest uppercase text-left">Bakım</span><p className="text-base text-slate-700">{item.bakim_turu}</p></div>
                  <div className="flex flex-col text-left"><span className="text-[11px] text-slate-300 mb-2 tracking-widest uppercase text-left">Şehir</span><p className="text-base text-slate-700">{item.sehir}</p></div>
                  <div className="flex flex-col text-left"><span className="text-[11px] text-slate-300 mb-2 tracking-widest uppercase font-black text-left">Zaman</span><p className="text-base text-slate-500">{item.tarih || "Şubat 2026"}</p></div>
                  <div className="flex flex-col items-end md:items-start text-left"><span className="text-[11px] text-slate-300 mb-2 tracking-widest uppercase text-left text-left">Tutar</span><p className="text-4xl font-black text-blue-700 tracking-tighter text-left">{item.ekran_fiyat}</p></div>
                </div>
            </div>
            {acikKartId === item.id && (
              <div className="p-12 bg-slate-50 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                <div className="space-y-3 uppercase font-bold text-left">
                  ...
                  <p className="text-sm">Motor: <span className="text-slate-900">{item.motor || '-'}</span></p>
                  <p className="text-sm">KM: <span className="text-slate-900">{item.km}</span></p>
                </div>
                <div className="space-y-3 uppercase font-bold text-left">
                  <p className="text-[11px] font-black text-slate-400 tracking-widest border-b pb-2 mb-4 text-left">Servis Bilgisi</p>
                  <p className="text-sm text-left font-black">Servis: <span className="text-slate-900">{item.servis_adi}</span></p>
                  <p className="text-sm text-left font-black">Paylaşan: <span className="text-blue-700 font-black">{item.kullanici_bas_harf}</span></p>
                </div>
      ...
      <section className="max-w-5xl mx-auto px-6 mt-32 mb-20 pt-20 border-t border-slate-200 text-left">
        <div className="flex justify-between items-center mb-16 text-left">
          ...
          <Link href="/blog" className="text-xs font-black text-blue-700 uppercase tracking-widest flex items-center gap-2 text-left text-left text-left text-left text-left text-left">Tümünü Gör <ArrowRight size={20}/></Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
          <Link href="/blog/istanbul-honda-bakim-fiyatlari-2026" className="group text-left">
            <div className="bg-gradient-to-br from-[#0f172a] to-[#1e293b] aspect-video rounded-[3rem] mb-8 overflow-hidden relative shadow-xl group-hover:-translate-y-2 transition-all">
          ...
          <Link href="/blog/fiat-egea-periyodik-bakim-tablosu-2026" className="group text-left">
            <div className="bg-gradient-to-br from-[#1e293b] to-[#334155] aspect-video rounded-[3rem] mb-8 overflow-hidden relative shadow-xl group-hover:-translate-y-2 transition-all">
               <div className="absolute bottom-8 left-10 text-left">
                 ...
                 <h3 className="text-3xl font-black text-white leading-tight italic tracking-tight uppercase text-left text-left text-left text-left text-left text-left">Egea Bakım Tablosu</h3>
               </div>
            </div>
          </Link>
        </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-20 px-8 text-left">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-left">
          <div className="text-left text-left text-left">
            <span className="text-3xl font-black italic text-slate-800 tracking-tighter uppercase block mb-2 text-left text-left">bakımım<span className="text-blue-700 text-left text-left text-left">.com</span></span>
