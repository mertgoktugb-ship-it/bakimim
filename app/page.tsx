...
import { Car, MapPin, Search, Calendar, ChevronDown, ChevronUp, TrendingUp, ShieldCheck, BadgePercent, Database, Edit3, Save, X, Check, Info, Mail, Zap, Settings, Activity } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] text-left pb-20">
      <nav className="bg-white border-b border-slate-200 px-8 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
           <a href="/" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col leading-tight">
                <span className="text-3xl font-black text-slate-800 tracking-normal italic">bakımım<span className="text-blue-700">.com</span></span>
                <span className="text-[10px] font-bold text-slate-400 tracking-[0.3em] mt-1 uppercase">Şeffaf Servis Rehberi</span>
              </div>
           </a>
           <div className="flex items-center gap-4">
              <button className="hidden md:flex items-center gap-2 text-xs font-black text-slate-600 hover:text-blue-700 transition-colors px-4 py-2 uppercase tracking-widest border border-slate-200 rounded-xl">
      ...
      {adminModu && (
        <div className="bg-orange-50 border-b border-orange-200 p-4 sticky top-[82px] z-40 flex justify-center gap-4">
      ...
      <div className="bg-[#0f172a] py-20 px-6 relative overflow-hidden">
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tighter uppercase italic">FİYAT <span className="text-blue-500">KIYASLA</span></h1>
          ...
          <div className="bg-white p-4 rounded-[2.5rem] shadow-2xl grid grid-cols-1 md:grid-cols-4 gap-4 text-left">
              ...
              <button onClick={sorgula} className="bg-blue-700 hover:bg-blue-800 text-white font-black rounded-2xl py-4 flex items-center justify-center gap-3 uppercase tracking-widest shadow-xl transition-all active:scale-95 text-lg">
      ...
      {sonuclar.length > 0 && (
        <div className="max-w-4xl mx-auto px-6 -mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 relative z-20">
            <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100 text-center">
              ...
              <p className="text-[11px] font-bold text-blue-500 mt-2 uppercase italic">Son 6 Ay Piyasa Verisi</p>
            </div>
            <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100 text-center">
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center justify-center gap-2">
              ...
              <p className="text-[11px] font-bold text-emerald-500 mt-2 uppercase italic">Son 6 Ay Piyasa Verisi</p>
            </div>
        </div>
      )}
            ...
            <div key={item.id} className={`bg-white rounded-[2.5rem] border overflow-hidden transition-all shadow-sm hover:shadow-md ${isEditing === item.id ? 'border-orange-500 ring-8 ring-orange-50' : 'border-slate-200'}`}>
              {isEditing === item.id ? (
                <div className="p-10 space-y-8 text-left">
                  <div className="flex justify-between items-center border-b border-orange-100 pb-6">
                    ...
                    <div className="flex gap-3">
                       ...
                       <button onClick={degisikligiKaydet} className="bg-emerald-500 text-white px-8 py-3 rounded-2xl font-black flex items-center gap-2 hover:bg-emerald-600 shadow-lg shadow-emerald-100"><Check size={22}/> KAYDET</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="flex flex-col gap-2">
                      <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Bakım Ücreti</label>
                    ...
                    <div className="col-span-full flex flex-col gap-2">
                      ...
                      <textarea rows={4} value={editForm.not} onChange={(e) => setEditForm({...editForm, not: e.target.value})} className="p-4 bg-slate-50 border-0 rounded-2xl font-bold text-base outline-none focus:ring-4 ring-emerald-100" />
                    </div>
                  </div>
                </div>
              ) : (
                <div className="relative group">
                  {adminModu && (
                    <button onClick={(e) => { e.stopPropagation(); editBaslat(item); }} className="absolute right-8 top-8 z-30 bg-orange-500 text-white px-6 py-2.5 rounded-2xl shadow-xl opacity-0 group-hover:opacity-100 transition-all flex items-center gap-2 text-xs font-black hover:bg-orange-600">
                  ...
                  <div onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)} className="p-8 md:p-10 flex flex-col md:flex-row items-center cursor-pointer select-none text-left">
                    <div className="md:w-64 text-left mr-10">
                      <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase mb-4 inline-block tracking-widest shadow-sm ${item.yetkili_mi === 'Evet' ? 'bg-blue-700 text-white' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
                    ...
                    <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 mt-8 md:mt-0 w-full text-left">
                      ...
                      <div className="flex flex-col"><span className="text-[11px] font-black text-slate-300 uppercase mb-2 tracking-[0.2em]">Tarih</span><div className="flex items-center gap-2 text-base font-bold text-slate-500"><Calendar size={18} className="text-slate-200"/> {item.tarih}</div></div>
                      <div className="flex flex-col items-end md:items-start"><span className="text-[11px] font-black text-slate-300 uppercase mb-2 tracking-[0.2em]">Tutar</span><p className="text-3xl font-black text-blue-700 tracking-tighter">{item.ekran_fiyat}</p></div>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                    <div className="space-y-5">
                      ...
                      <div className="space-y-4 text-xs font-bold text-slate-500 uppercase">
                        <p className="flex justify-between">MODEL YILI: <span className="text-slate-900">{item.model_yili || '-'}</span></p>
                        <p className="flex justify-between">KİLOMETRE: <span className="text-slate-900">{item.km} KM</span></p>
                        <p className="flex justify-between">MOTOR: <span className="text-slate-900">{item.motor || '-'}</span></p>
                      </div>
                    </div>
                    <div className="space-y-5">
                      <h4 className="text-[12px] font-black text-slate-800 uppercase tracking-[0.3em] border-b border-slate-200 pb-3">İşlemler</h4>
                      <div className="flex flex-wrap gap-2.5">
                        ...
                        )) : <span className="text-slate-400 italic text-xs">Ayrıntı dökülmemiş.</span>}
                      </div>
                    </div>
                    <div className="space-y-5">
                      ...
                      <div className="space-y-4 text-left">
                         ...
                         <div className="bg-blue-700 text-white p-6 rounded-[2rem] italic text-[13px] leading-relaxed font-medium shadow-2xl shadow-blue-200 relative">
                           "{item.temiz_not}"
                         </div>
                      </div>
                    </div>
