...
const CustomSelect = ({ label, value, options, onChange, icon: Icon, isDark }: any) => {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      {/* KOYU MOD METİN GÖRÜNÜRLÜĞÜ ÇÖZÜLDÜ */}
...
export default function BakimimApp() {
  ...
  const kategorizeEt = (metin: string) => {
    ...
    if (m.includes("ağır") || m.includes("triger") || m.includes("revizyon") || m.includes("şanzıman") || m.includes("rektifiye") || m.includes("sandık")) return "AĞIR BAKIM";
    if (m.includes("alt takım") || m.includes("yürüyen") || m.includes("fren") || m.includes("balata") || m.includes("disk") || m.includes("rot")) return "ALT TAKIM & YÜRÜYEN AKSAM";
    return "PERİYODİK BAKIM";
  };
    ...
    try {
      ...
      if (data) {
        const valideEdilmisData = data.filter(item => item.marka && item.model && item.fiyat).map(item => ({
          ...
          bakim_kategorisi: kategorizeEt(item.bakim_turu || ""), // UNDEFINED HATASININ ÇÖZÜMÜ BURADA
  ...
  const sorgula = () => {
    const temelFiltre = duzenlenenVeri.filter(item => {
      ...
      if (secilenBakimKategorisi === "Periyodik Bakım") kUygun = m.includes("bakım") || m.includes("yağ") || m.includes("periyodik");
      ...
      else if (secilenBakimKategorisi === "Ağır Bakım") kUygun = m.includes("ağır") || m.includes("triger") || m.includes("revizyon");
  ...
  const getKategoriLinkEki = (item: any) => {
    const m = (item.bakim_turu || "").toLocaleLowerCase('tr-TR');
    if (m.includes("ağır") || m.includes("triger") || m.includes("revizyon")) return "/agir-bakim-fiyatlari";
    if (m.includes("alt takım") || m.includes("balata") || m.includes("fren") || m.includes("disk")) return "/alt-takim-yuruyen-aksam-fiyatlari";
    ...
    <main className={`min-h-screen pb-20 text-left relative font-sans transition-colors duration-500 ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-[#F8FAFC] text-slate-800'}`}>
      ...
      {veriYukleniyor ? (
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* İSTEDİĞİNİZ KUSURSUZ KART TASARIMI */}
            ...
            {sonuclar.length > 0 ? sonuclar.map((item) => (
              <div key={item.id} className={`rounded-[2.5rem] border overflow-hidden shadow-sm hover:border-yellow-400 transition-all flex flex-col ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-slate-200 text-slate-800'}`}>
                <div onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)} className="p-8 cursor-pointer flex-1 flex flex-col">
                  ...
                  {/* Marka & Model & Bakım Tipi + KM (KUTUCUKLU) */}
                  <div className="mb-6 text-left">
                    ...
                    {/* TAM İSTEDİĞİN GİBİ: Bakım Tipi ve Kutu İçinde Detay */}
                    <div className="space-y-4 mb-8 text-left mt-6">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-yellow-600 dark:text-yellow-500 uppercase font-black tracking-widest mb-2 flex items-center gap-2">
                           <Layers size={14} /> {item.bakim_kategorisi} {item.km ? `(${item.km.toLocaleString('tr-TR')} KM)` : ''}
                        </span>
                        {/* UNDEFINED HATASINI ÇÖZEN, ŞEFFAF KUTU YAPISI */}
                        <div className={`px-4 py-2.5 rounded-xl border text-xs font-bold uppercase w-fit tracking-tight ${isDarkMode ? 'border-slate-700 bg-slate-800/50 text-slate-300' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                           {item.bakim_turu_format}
                        </div>
                      </div>
                      
                      {/* ŞEHİR VE DURUM GRID'İ */}
                      <div className="grid grid-cols-2 gap-4 mt-6">
                        <div className="flex flex-col"><span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Şehir</span><p className="text-xs font-bold truncate uppercase">{item.sehir}</p></div>
                        <div className="flex flex-col">
                           <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Durum</span>
                           <p className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1 ${item.fatura_onayli ? 'text-emerald-500' : 'text-blue-500'}`}>
                             {item.fatura_onayli ? 'BELGE ONAYLI' : 'KULLANICI BEYANI'}
                           </p>
                        </div>
                      </div>
                    </div>
                ...
                {/* YALIN VE ŞIK AÇILIR MENÜ DETAYI (Gönderdiğin koddaki gibi) */}
                {acikKartId === item.id && (
                  <div className={`p-8 border-t space-y-6 animate-in slide-in-from-top-4 duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                    <div className="space-y-4 text-left text-slate-800">
                      <div><span className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Bakım Detayı</span><p className="text-sm font-bold text-slate-700 dark:text-slate-300 leading-snug uppercase">{item.bakim_turu_format}</p></div>
                      <div className="grid grid-cols-2 gap-4">
                        <div><span className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Motor</span><p className="text-sm font-bold uppercase dark:text-white">{item.yakit_motor || '-'}</p></div>
                        <div><span className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Kilometre</span><p className="text-sm font-bold uppercase dark:text-white">{item.km ? item.km.toLocaleString('tr-TR') : '-'} KM</p></div>
                      </div>
                    </div>
                    ...
                    <Link 
                       ...
                       className={`w-full py-4 rounded-2xl font-black text-[9px] uppercase tracking-[0.15em] flex items-center justify-center gap-2 transition-all border ${isDarkMode ? 'hover:bg-yellow-500 hover:text-slate-900 border-slate-600 text-slate-300' : 'hover:bg-yellow-500 hover:text-slate-900 border-slate-300 text-slate-600'}`}
                    >
                       TÜM KAYITLARI GÖR <ArrowRight size={14}/>
                    </Link>
                  </div>
                )}
              </div>
            ...
            <form onSubmit={veriyiGonder} className="p-10 space-y-8 text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                ...
                <div className="space-y-2 text-left md:col-span-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-left block mb-1">Servis Tipi</label>
