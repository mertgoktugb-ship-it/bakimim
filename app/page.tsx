  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-20 text-left relative">
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-5 sticky top-0 z-50 flex justify-between items-center shadow-sm">
           ...
           <div className="flex items-center gap-4">
              <button onClick={() => setAdminModu(!adminModu)} className={`text-[10px] font-black px-5 py-2.5 rounded-xl border transition-all ${adminModu ? 'bg-red-600 text-white border-red-700 shadow-lg' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                {adminModu ? 'YÖNETİCİ MODU AKTİF' : 'GİRİŞ'}
              </button>
      ...
      <section className="max-w-5xl mx-auto px-6 space-y-6 mt-12 text-left">
        {sonuclar.map((item) => (
          <div key={item.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm hover:border-blue-300 transition-all group">
            <div className="p-8 md:p-12 flex flex-col md:flex-row items-center text-left relative">
                
                {/* YÖNETİCİ BUTONLARI (SADECE ADMİN MODUNDA GÖRÜNÜR) */}
                {adminModu && (
                  <div className="absolute top-6 right-8 flex gap-2">
                    <button className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors"><Edit3 size={16} /></button>
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 w-full font-black uppercase italic text-left cursor-pointer" onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)}>
                  ...
                  <div className="flex flex-col text-left"><span className="text-[11px] text-slate-300 mb-2 uppercase">Tarih</span><p className="text-base text-slate-500">{item.tarih}</p></div>
            ...
            {acikKartId === item.id && (
              <div className="p-10 bg-slate-50 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm italic text-left animate-in slide-in-from-top-4">
                ...
                <div className="bg-gradient-to-br from-[#0f172a] to-[#1e293b] text-white p-6 rounded-[2rem] shadow-lg text-left">"{item.not || "Veri doğrulanmış fatura kaydıdır."}"</div>
              </div>
            )}
          </div>
        ))}
      ...
      <footer className="bg-white border-t border-slate-200 py-20 px-8 text-left">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-left text-left text-left">
          ...
          <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">KörNoktaTv İş Birliğiyle</div>
        </div>
      </footer>
    </main>
  );
