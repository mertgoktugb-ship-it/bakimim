    ...
    const veriGetir = async () => {
      ...
      if (data) {
        const duzenlenmisData = data.map(item => ({
          ...
          return m.includes("alt takım") || m.includes("fren") || m.includes("balata") || m.includes("disk");
        });
              ...
              <div onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)} className="p-8 flex-1 flex flex-col text-left cursor-pointer relative">
                ...
                <div className="grid grid-cols-2 gap-4 mb-2 text-left">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className={`text-[10px] sm:text-xs font-black uppercase tracking-tighter ${item.fatura_onayli ? 'text-emerald-500' : 'text-blue-500'}`}>{item.fatura_onayli ? 'Belge Destekli Kullanıcı Bildirimi' : 'Kullanıcı Beyanı'}</p>
                  </div>
                </div>
                {acikKartId === item.id && (
