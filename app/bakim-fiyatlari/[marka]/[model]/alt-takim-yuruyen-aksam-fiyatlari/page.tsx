...
// 5 KLASÖR GERİ ÇIKAN DOĞRU YOL (ALT KATEGORİLER İÇİN)
...
export default function AltKategoriDetaySayfasi() {
  ...
  // BU SAYFA ALT TAKIM BAKIMLARINI GÖSTERİR
  const aktifKategori: string = "alt-takim";
  ...
  const veriCek = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        let hedefKategori = "Periyodik Bakım";
        if (pathname.includes("agir")) hedefKategori = "Ağır Bakım";
        else if (pathname.includes("alt-takim")) hedefKategori = "Alt Takım & Yürüyen Aksam";
        ...
        const filtrelenmis = data.map(item => ({
          ...
          const kategoriMatch = item.bakim_kategorisi === hedefKategori;
          return markaMatch && modelMatch && kategoriMatch;
        });
  ...
  }, [params.marka, params.model, pathname]);

  useEffect(() => { veriCek(); }, [veriCek]);

  let sayfaBaslik = "PERİYODİK BAKIM";
  if (pathname.includes("agir")) sayfaBaslik = "AĞIR BAKIM";
  else if (pathname.includes("alt-takim")) sayfaBaslik = "ALT TAKIM & YÜRÜYEN AKSAM";
  ...
  if (yukleniyor) {
    return (
      <div className="min-h-screen flex items-center justify-center font-black text-yellow-500 uppercase italic animate-pulse">
        VERİLER SENKRONİZE EDİLİYOR...
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6 mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20 items-start">
          {siraliSonuclar.length > 0 ? siraliSonuclar.map((item) => {
            ...
            <div className={`col-span-full text-center py-32 rounded-[3rem] border border-dashed ${isDarkMode ? "bg-slate-900 border-slate-800 text-slate-600" : "border-slate-200 text-slate-400"}`}>
