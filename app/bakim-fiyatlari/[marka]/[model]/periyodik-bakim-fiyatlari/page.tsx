...
export default function PeriyodikBakimSayfasi({ params }: { params: any }) {
  ...
  useEffect(() => {
    const veriGetir = async () => {
      try {
        ...
        // URL'den gelen 'tiggo-7-pro' ifadesini 'tiggo 7 pro' haline getiriyoruz
        // ilike kullanarak büyük/küçük harf sorununu da aşıyoruz
  ...
  if (yukleniyor || !resolvedParams) {
    return <div className={`min-h-screen flex items-center justify-center font-black text-yellow-500 italic ${isDarkMode ? 'bg-slate-950' : 'bg-[#F8FAFC]'}`}>VERİLER ÇEKİLİYOR...</div>;
  }
      ...
      <div className="py-16 px-6 border-b bg-[#0f172a] text-white">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-slate-400 font-bold uppercase text-[11px] tracking-[0.3em] mt-4 flex items-center gap-2">
            <Zap size={14} className="text-yellow-500" /> TOPLAM {kayitlar.length} DOĞRULANMIŞ KAYIT
          </p>
        </div>
      </div>
            ...
            <div key={item.id} className={`rounded-[2.5rem] border overflow-hidden shadow-sm transition-all flex flex-col h-fit group ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} ${acikKartId === item.id ? 'ring-2 ring-yellow-500 shadow-xl' : ''}`}>
              <div onClick={() => setAcikKartId(acikKartId === item.id ? null : item.id)} className="p-8 flex-1 flex flex-col text-left cursor-pointer relative">
                ...
                <div className="mb-6">
                  ...
                  <div className="mt-4 space-y-1">
                    <div className="flex items-center gap-2 text-yellow-700 dark:text-yellow-500"><Layers size={14} /><span className="text-[10px] font-black uppercase tracking-widest">PERİYODİK BAKIM ({item.km?.toLocaleString('tr-TR')} KM)</span></div>
                ...
                {/* Alt detaylar (Notlar vs) açılınca buraya gelecek */}
              </div>
              <div className="p-8 pt-0 flex flex-col gap-4 mt-auto">
                <div className={`pt-6 border-t flex justify-between items-end ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                  <div><span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1 block">Toplam Tutar</span><p className="text-3xl font-black text-yellow-600 tracking-tighter">{item.fiyat?.toLocaleString('tr-TR')} TL</p></div>
                  <div className="text-[10px] font-black text-slate-400 uppercase italic">{item.tarih ? item.tarih.split('-').reverse().join('.') : '-'}</div>
                </div>
              </div>
            </div>
          )) : (
            <div className="col-span-full text-center py-20 border-2 border-dashed border-slate-200 rounded-[2rem]">
               <p className="font-black text-slate-400 uppercase italic italic">Bu model için henüz kayıt bulunmuyor.</p>
            </div>
          )}
        </div>
      </div>
